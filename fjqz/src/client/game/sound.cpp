...
constexpr std::size_t n_of_channels = 32;
constexpr int sample_rate = 44100;
constexpr int sound_buffer_size = 1024;
// we need 2 channels, because we it for timer as well
constexpr std::size_t bell_channel = 0;
constexpr std::size_t timer_channel = 1;

// number of channels reserved for sound sources
constexpr std::size_t source_channels = 8;
constexpr std::size_t source_channel_start = timer_channel + 1;
constexpr std::size_t source_channel_last = source_channel_start + source_channels - 1;
constexpr std::size_t UI_sound_channels = 2;
constexpr std::size_t UI_sound_channel_start = source_channel_last + 1;
constexpr std::size_t UI_sound_channel_last = UI_sound_channel_start + UI_sound_channels - 1;
constexpr std::size_t n_reserved_channels = UI_sound_channel_last + 1; // sources, bell, timer and UI
...
//
std::vector<std::shared_ptr<music_track>> current_track_list;
std::shared_ptr<music_track> current_track;
unsigned int current_track_index = 0;
std::shared_ptr<music_track> previous_track;

std::vector<std::shared_ptr<music_track>>::const_iterator find_track(const music_track& track)
{
	return std::find_if(current_track_list.begin(), current_track_list.end(),
						[&track](const std::shared_ptr<const music_track>& ptr) { return *ptr == track; }
	);
}
...
{
	LOG_INFO("Audio", "Considering %s", id.c_str());
...
static std::shared_ptr<music_track> choose_track()
{
...
driver_status driver_status::query()
{
	...
	if(mix_ok) {
		...
		res.chunk_size = sound_buffer_size;
	}
...
bool init_sound()
{
	LOG_INFO("Audio", "Initializing audio...");
	...
	if(!mix_ok)
	{
		if(Mix_OpenAudio(sample_rate, MIX_DEFAULT_FORMAT, 2, sound_buffer_size) == -1)
		{
			mix_ok = false;
			LOG_ERROR("Audio", "Could not initialize audio: %s", Mix_GetError());
			return false;
		}
		...
		set_sound_volume(CONFIG_MGR->get_window_value("sound_volume") /*preferences::sound_volume()*/);
		set_UI_volume(CONFIG_MGR->get_window_value("ui_volume") /*preferences::UI_volume()*/);
		set_music_volume(CONFIG_MGR->get_window_value("music_volume") /*preferences::music_volume()*/);
		set_bell_volume(CONFIG_MGR->get_window_value("bell_volume") /*preferences::bell_volume()*/);
...
{
	if(false /*preferences::stop_music_in_background()*/ && true)
	{
