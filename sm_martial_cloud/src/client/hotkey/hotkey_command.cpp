...
namespace
{
	...
	/*
	 * 所有静态热键
	 * 该数组应该使用与Hotkey_command枚举具有相同的条目数，由于HOTKEY_NULL是
	 * 所述枚举中最后一项，我们可以用某个索引来动态调整数组的大小
	 */
		...
		std::set<Hotkey_command> toggle_commands {HOTKEY_SCROLL_UP,
			HOTKEY_SCROLL_DOWN, HOTKEY_SCROLL_LEFT, HOTKEY_SCROLL_RIGHT};

		// 包含Hotkey_list和所有当前活动的wml菜单热键的副本
		std::vector<Hotkey_command> known_hotkeys;

		// known_hotkeys的索引图，由于known_hotkeys以Hotkey_list开头，因此
		// 他们也是Hotkey_list的索引
		std::map<std::string, size_t> command_map;
...
namespace hotkey
{
	Scope_changer::scope_changer() : prev_scope_active_(scope_active) {}

	Scope_changer::~scope_changer() { scope_active_ = prev_scope_active_; }

	void deactivate_all_scopes()
	{
		for(int i = 0; i < SCOPE_COUNT; ++i)
			scope_active[i] = false;
	}

	void set_scope_active(Scope s, bool set) { scope_active[s] = set; }

	void set_active_scopes(Hk_scopes s) { scope_active = s; }
	...
	bool remove_wml_hotkey(const std::string& id)
	{
		...
		if(command.id == HOTKEY_NULL)
		{
			LOG_G << "remove_wml_hotkey: command with id=" + id + 
				" doesn't exist\n";
			return false;
		}
		else if(command.id != HOTKEY_WML)
		{
			LOG_G << "remove_wml_hotkey: command with id =" + id + 
		...
		{
			LOG_G << "removeing wml hotkey with id=" + id + '\n';

			for(auto it = known_hotkeys.rbegin(); it != know_hotkeys.rend();
					++it)
			{
				if(it->command == id)
				{
					know_hotkeys.erase(std::next(it).base());
					break;
				}
			}

			// comand_map可能是错误的，所以我们需要重建
			command_map.clear();

			for(size_t i = 0; i < known_hotkeys.size(); ++i)
				command_map[known_hotkeys[i].command] = i;
			return true;
		}
	}
	...
	{
		return get_hotkey_command(id).id != HOTKEY_NULL;
	}

	void add_vml_hotkey(const std::string& id, const std::string& description, const Config& default_hotkey)
	{}

	Hotkey_command::Hotkey_command(const Hotkey_command_temp& temp_command):
		id(temp_command.id),
		command(temp_command.command),
		description(temp_command.description, "lib"),
		...
		tooltip(temp_command.tooltip, "lib"); {}

	Hotkey_command::Hotkey_command(Hotekey_command cmd, 
			const std::string& id, const std::string& desc, bool hid, 
			bool tog, Hk_scopes scop, Hotkey_category cat, 
			const std::string toolp):
		id(cmd), command(id), description(desc), hidden(hid), toggle(tog),
		scope(scop), category(cat), tooltip(toolt) {}
	...
	bool Hotkey_command::null() const
	{
		if(id == HOTKEY_NULL || command == "null")
		{
			...
			if(id == null_cmd.id && command == null_cmd.command &&
					scope == null_cmd.scope && description == 
					null_cmd.description)
				return true;
			else
			{
				ERR_G << "The hotkey command seems to be the null command\
					but it is not 100% sure. This shouldn't happen";
				return true;
			}
		}
		return false;
	...
	const Hotkey_command& Hotkey_command::get_command_by_command(Hotkey_command command)
	{
		...
		ERR_G << "\"get_command_by_command\" returned get_hotkey_null()\
			because no hotkey_command had the requested number" << command;

		return get_hotkey_null();
	}

	const Hotkey_comand& get_hotkey_null()
	{
		// 他是数组中的最后一项，并且Hotkey_list和known_hotkeys中的索引是
		// 相同的。
		return known_hotkeys[hotkey_list.size() - 1];
	}
	...
	void init_hotkey_commands();
	{
		...
		// 为内置热键预留足够的空间
		known_hotkeys.reserve(hotkey_list.size() + 20);

		size_t i = 0;
		for(Hotkey_command_temp& cmd : hotkey_list)
		{
			know_hotkeys.emplace_back(cmd);
			comand_map[cmd.command] = i++;
		}
	}

	void clear_hotkey_commands() { comand_map.clear(); }

	Hotkey_command get_id(const std::string& command)
	{
		return get_hotkey_command(comand).id;
	}
}
