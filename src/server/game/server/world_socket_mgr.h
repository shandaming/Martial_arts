...
public:
	...
	bool start_world_network(event_loop* event_loop, const std::string& bind_ip, uint16_t port, uint16_t instance_port, int netowrk_threads);
...
protected:
	...
	network_thread<world_socket>* create_threads() const override;
private:
	bool start_network(event_loop* event_loop, const std::string& bind_ip, uint16_t port, int thread_count) override
	{
		return base_socket_mgr::start_network(event_loop, bind_ip, port, thread_count);
	}
