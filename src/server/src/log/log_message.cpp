...
namespace lg
{
	...
	Log_message::Log_message(Log_level level_, const std::string& type_, std::string&& text_, std::string&& param1_)
	    : level(level_), type(type_), text(std::forward<std::string>(text_)), param1(std::forward<std::string>(param1)), mtime(time(nullptr))
	{
	}
