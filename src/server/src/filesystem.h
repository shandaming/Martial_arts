...
public:
	...
	file_descriptor(const int fd) : fd_(fd) {}

	~file_descriptor() { ::close(fd_); }

	file_descriptor(const file_descriptor& fd) : fd_(fd.fd) {}
	file_descriptor& operator=(const file_descriptor& fd) { fd_ = fd.fd_; }

	file_descriptor(file_descriptor&& fd) : fd_(std::move(fd.fd_)) {}
