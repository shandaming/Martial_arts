...
#define SQUARE_MAIN_MAP_COORD_COUNT (MAIN_MAP_COORD_COUNT * 2)
...
auto load_main_map_data(std::string_view file)
{
	...
	ASSERT(data.size() >= SQUARE_MAIN_MAP_COORD_COUNT, "Failed to load main map data.");

	std::vector<int16_t> result(SQUARE_MAIN_MAP_COORD_COUNT, 0);
	memmove(result.data(), data.data(), SQUARE_MAIN_MAP_COORD_COUNT);
...
auto load_main_map_texture(std::string_view file)
{
	...
	std::vector<grid> result(SQUARE_MAIN_MAP_COORD_COUNT);
	...
	for (int i = 0; i < square_map_data.size(); i++)
	{
		...
		if (pic > 0)
		{
			result[i].texture = TextureManager::getInstance()->loadTexture("mmap", pic);
			auto& grid = result[i];
